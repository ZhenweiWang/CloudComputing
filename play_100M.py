...
with open('twitter.csv', encoding='utf-8') as f:
    ...
    for singel_chunk in read_line_size(f, 100*1024*1024):
        if comm_rank == 0:
        ...
        print('rank %d end at %s' %(comm_rank,n_e_t_record))
        print('Processing time for node %d is %s\n' %(comm_rank,n_time_consume))
